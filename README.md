...
Pour lancer le projet il faut d'abord se placer dans le dossier `api/` et crée un fichier `.env` a partir du fichier `.env-template` (equivalent du .env.example), ensuite il faut lancer la commande `docker compose up --build` pour construire les images et lancer les conteneurs.
